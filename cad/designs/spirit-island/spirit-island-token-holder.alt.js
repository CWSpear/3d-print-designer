...
function makeHole(x, y, size) {
  ...
  return translate(
    [THICKNESS + x * (ROW_UNIT_WIDTH + BORDER_WIDTH), THICKNESS + y * (CELL_HEIGHT + BORDER_WIDTH), BORDER_WIDTH],
    difference(
      cube({
...
function makeLidLip(offset = 0) {
  ...
  //   translate(
  //     [0, THICKNESS, 0],
  //     cube({ size: [BOX_WIDTH_WITH_WIGGLE - (THICKNESS), BOX_HEIGHT_WITH_WIGGLE - (THICKNESS * 2), THICKNESS + EXTRA] })
  //   )
  // );
...
function lid() {
  ...
  return translate(
    ...
    difference(
      translate(
        [0, 0, EXTRA],
        cube({ size: [BOX_WIDTH_WITH_WIGGLE - offset, BOX_HEIGHT_WITH_WIGGLE - offset - offset, LIPNESS] }),
      ),
      makeLidLip(offset),
      translate(
        [BUTTON_DISTANCE_FROM_EDGE, BOX_HEIGHT_WITH_WIGGLE / 2 - BUTTON_RADIUS - 3, 0],
        cylinder({ r: BUTTON_RADIUS, h: LIPNESS * 0.6, fn: 64 }),
      ),
      translate(
        [BUTTON_DISTANCE_FROM_EDGE, BOX_HEIGHT_WITH_WIGGLE / 2 + BUTTON_RADIUS + 3, 0],
        cylinder({ r: BUTTON_RADIUS, h: LIPNESS * 0.6, fn: 64 }),
